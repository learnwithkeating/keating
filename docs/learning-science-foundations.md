...
**P20. Deepening, not accelerating — structurally.** At least one night of sleep between first exposure and first verification ("learned today, verified tomorrow"; Newbury 2021). Goals early in a topic are process goals, not outcome goals (Winters & Latham 1996). Retention horizons come from the learner's mission, which routinely outruns the course calendar: MISSION.md carries a Horizon — a date, a duration, or indefinitely — and the delayed check's first gap is derived from it at the low end of Cepeda's 10-30% band, clamped so it never falls below the consolidation rule or drifts past the point of being useful feedback. A mission that states none keeps the platform's own constant: silence is not an instruction, and a horizon guessed wrongly moves every review the learner gets.
